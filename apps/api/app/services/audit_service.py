...
# (booking, item, item_catalog, item_photo, payment, staff, customer).
...
def record_audit_log(
    ...
    manual payment recording), and an audit row should never be persisted
    on its own if that surrounding change fails to commit. Just db.add();
    the caller's existing commit() picks it up.
    """
    db.add(
        AuditLog(
            actor_id=actor_id,
